...
Load : nạp dữ liệu vào và dựng cây 
Print: In cây ra màn hình 
AddLeftChild  : thêm nút con trái  (nếu chưa tồn tại) vào nút định danh  trên cây nếu  tồn tại 
AddRightChild  : thêm con phải  (nếu chưa tồn tại) vào nút có định danh  vào cây nếu   tồn tại 
Find : tìm nút có định danh 
Count: Đếm số nút của cây 
FindLeaves: In ra màn hình các nút lá của cây 
Height : in ra màn hình chiều cao của nút  (nếu tồn tại) 
Quit: thoát khỏi chương trình

Dữ liệu về cây được nhập vào với định dạng như sau:  Mỗi cặp 3 số nguyên dương t, u, v trong đó u và v 
(nếu khác -1) sẽ tương ứng là  con trái và con phải của t. Việc nhập kết thúc bởi -2
*/
...
void printChildren(Node *p)
{
    ...
    if (p->rightChild == NULL)
        printf(" Node %d does not has rightChild\n", p->id);
    else
        printf(", RightChild = %d\n", p->rightChild->id);
}
