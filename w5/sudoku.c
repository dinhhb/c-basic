...
// This function finds an entry in the grid that is still unassigned
...
/* Searches the grid to find an entry that is still unassigned. If found, the reference parameters row, col will be set to the location that is unassigned, and 1 is returned. If no unassigned entries remain, 0 is returned. */
int FindUnassignedLocation(int grid[N][N], int *row, int *col)
{
    for (*row = 0; *row < N; (*row)++)
        for (*col = 0; *col < N; (*col)++)
...
/* Returns 1 if an assigned entry in the specified row matches the given number, 0 otherwise */
...
/* Returns 1 if an assigned entry in the specified column matches the given number, 0 otherwise */
...
/* Returns 1 if an assigned entry within the specified 3x3 box matches the given number, 0 otherwise */
...
/* Returns 1 if it will be legal to assign num to the given row, col location, 0 otherwise */
int isSafe(int grid[N][N], int row, int col, int num)
{
